...
class FilterObs(gym.ObservationWrapper):
            ...
            {agent_id: gym.spaces.Dict(
                    {
                        ...
                        # 所有观测放在一起的向量观测
                        "brief_final_obs": gym.spaces.Box(low=-1e10, high=+1e10, shape=(168,), dtype=np.float32, ),
                        # transformer的二维观测
                        "brief_transformer_pos_and_heading_2d": gym.spaces.Box(low=-1e10, high=+1e10, shape=(11,9), dtype=np.float32, ),
                        # 单独的向量观测
                        "brief_together_obs": gym.spaces.Box(low=-1e10, high=+1e10, shape=(69,), dtype=np.float32, ),
                        # 车辆辨别观测
    ...
    def observation(self, obs: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        for agent_id, agent_obs in obs.items():
            ...
            # 获取goal的位置和方向
            goal_pos_and_heading = [agent_obs["mission"]["goal_pos"][0], agent_obs["mission"]["goal_pos"][1], \
                                    goal_heading[0] , agent_obs["mission"]["goal_pos"][0] - pos_and_heading[0], \
                                    agent_obs["mission"]["goal_pos"][1] - pos_and_heading[1], \
                                    goal_heading[0] - pos_and_heading[2], goal_distance[0]]      
