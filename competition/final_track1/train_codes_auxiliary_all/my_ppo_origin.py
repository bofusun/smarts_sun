...
class PPO():
    ...
    def train_step(self):
        ...
        for _ in range(self.buffer_capacity):
            ...
            with torch.no_grad():
                ...
                critic_value, scenario_output = self.critic_net(all_vector_state, rgb_state, together_vector_state, trans_vector_state, judge_state)
        ...
        reached_pro = goal_count / len(return_rewards)
        ...
        with torch.no_grad():
            mu, sigma, action, log_prob, entropy = self.select_action(all_vector_state, rgb_state, together_vector_state, trans_vector_state, judge_state)
            return_value, scenario_output = self.critic_net(all_vector_state, rgb_state, together_vector_state, trans_vector_state, judge_state)
        ...
        for _ in range(self.epoch):
            for index in BatchSampler(SubsetRandomSampler(range(self.buffer_capacity * self.env_num)), self.batch_size, drop_last=False):
                ...
                return_value, scenario_output = self.critic_net(all_vector_states[index], rgb_states[index], together_vector_states[index], trans_vector_states[index], judge_states[index])
                # 得到ratio
        ...
        self.replay_buffer = []
        return np.mean(return_rewards), np.mean(action_losses), np.mean(value_losses), np.mean(entropies), np.mean(kls), np.mean(lrs), reached_pro
        ...
        while True:
            ...
            if done:
                if info[0]["is_success"] == True:
                    reach_goal = True
                break
        return all_reward, reach_goal

    def eval_episodes(self):
        goal_count = 0
        return_rewards = []
        for i in range(self.eval_num):
            all_reward, reach_goal = self.eval_episode()
            return_rewards.append(all_reward)
            if reach_goal:
                goal_count += 1
        goal_pro = float(goal_count) / float(self.eval_num)
        self.logger.info('steps={}\t test_return_reward={:.5f}\t goal_pro={:.5f}'.format(self.counter, np.mean(return_rewards), goal_pro))
    ...
    def run_steps(self):
        ...
        if self.load_counter:
            ...
            reached_pros = data["reached_pros"]
            test_goal_pros = data["test_goal_pros"]
        ...
        while self.counter < self.max_steps:
            # 训练并记录训练数据
            return_reward, action_loss, value_loss, entropy, kl, lr, reached_pro = self.train_step()
            # 记录
            train_return_rewards.append(return_reward)
            reached_pros.append(reached_pro)
            ...
            self.logger.info(
                    ...
                    .format(self.counter, return_reward, reached_pro, action_loss, value_loss, entropy, kl, lr))
            t0 = time.time()
            if self.counter % self.eval_interval == 0:
                test_return_reward, test_goal_pro = self.eval_episodes()
                test_return_rewards.append(test_return_reward)
                test_goal_pros.append(test_goal_pro)
            if self.counter % self.log_interval == 0:
                save_info = {"train_return_rewards": train_return_rewards, "test_return_rewards": test_return_rewards, "reached_pros": reached_pros, \
                             "test_goal_pros":test_goal_pros, "action_losses": action_losses, "value_losses": value_losses, "entropies": entropies, \
