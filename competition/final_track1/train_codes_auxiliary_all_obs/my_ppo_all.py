...
def transition(all_vector_state, rgb_state, together_vector_state, trans_vector_state, judge_state, scenario_label, \
    ...
    return Transition(all_vector_state, rgb_state, together_vector_state, trans_vector_state, judge_state, scenario_label, \
                      action, torch.from_numpy(reward).to(device), log_prob, done, torch.from_numpy(1 - done).to(device), \
                      0, 0, critic_value)
...
class PPO():
    ...
    def reshape_obs_new(self, obs):
        ...
        for i in range(self.env_num):
            ...
            scenario_label.append(list(np.squeeze(obs[i]["brief_scenario_obs"],axis=0)))
        ...
        scenario_label = np.array(scenario_label, dtype=np.float32)
        return all_vector_obs, rgb_obs, togrther_vector_obs, trans_vector_obs, judge_obs, scenario_label
    ...
    def numpy_2_torch(self, all_vector_state, rgb_state, together_vector_state, trans_vector_state, judge_state, scenario_label):
        ...
        scenario_label = torch.from_numpy(scenario_label).long().to(self.device)
    ...
    def train_step(self):
        ...
        for _ in range(self.buffer_capacity):
            ...
            with torch.no_grad():
                ...
                # print("mu", mu)
                # print("scenario_output", scenario_output)
                # print("sigma", sigma)
                # print("action", action)
            ...
            for i, done_ in enumerate(done):
                if done_:
                    ...
                    self.reward_norms[i].reset()
                    print("index", i)
                    print("Agent_0", infos[i][0]["is_success"])
                    if infos[i][0]["is_success"] == True:
                        goal_count += 1
            # 存储数据
        ...
        reached_pro = float(goal_count) / float(len(return_rewards))
        self.logger.info('steps={}\t episode_num={:.5f}\t reached_num={:.5f}'.format(self.counter, float(len(return_rewards)), goal_count))
        self.counter += 1
        all_vector_state, rgb_state, together_vector_state, trans_vector_state, judge_state, scenario_label = self.numpy_2_torch(all_vector_state, rgb_state, together_vector_state, trans_vector_state, judge_state, scenario_label)
        # 生成动作
        with torch.no_grad():
        ...
        scenario_labels = torch.cat([t.scenario_label for t in self.replay_buffer], axis=0).view(-1).to(self.device)
        ...
        self.replay_buffer = []
        return np.mean(return_rewards), np.mean(action_losses), np.mean(value_losses), np.mean(actor_supervise_losses), \
...
if __name__ == "__main__":
    ...
    # 保存文件路径
    path_name = 'aux_01_origin_reward_step_00001_try'
