...
class PPO():
    def __init__(self, params):
        ...
        if params.actor_net_type == "mlp":
            self.actor_net = Mlp_Actor_model_action_pose2(self.actor_input_dim, self.action_dim, params.device).to(device=params.device)
        elif params.actor_net_type == "cnn1":
            self.actor_net = CNN_Actor_model_action_pose2(self.action_dim, params.device).to(device=params.device)
        # 添加Critic网络
    ...
    def train_step(self):
        ...
        for _ in range(self.buffer_capacity):
            ...
            # print("train pose_action", pose_action)
            # print("step reward", reward)
            # print("step done", done)
            # 转换状态
            next_action_state = self.get_action_obs(next_state)
    ...
    def eval_step(self, obs):
        ...
        # print("eval_action_state", action_state)
        # 生成动作
        with torch.no_grad():
