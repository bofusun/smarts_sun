...
class FilterObs(gym.ObservationWrapper):
    ...
    def __init__(self, env: gym.Env):
        ...
        self.observation_space = gym.spaces.Dict(
            {agent_id: gym.spaces.Dict(
                    {
                        # To make car follow the waypoints
                        "pos_and_heading": gym.spaces.Box(low=-1e10, high=1e10, shape=(4,), dtype=np.float32, ),
                        # rgb图像
                        ...
                        "brief_final_obs": gym.spaces.Box(low=-1e10, high=+1e10, shape=(106,), dtype=np.float32, ),
                        "brief_transformer_pos_and_heading_2d": gym.spaces.Box(low=-1e10, high=+1e10, shape=(11,5), dtype=np.float32, ),
                        "brief_together_obs": gym.spaces.Box(low=-1e10, high=+1e10, shape=(51,), dtype=np.float32, ),
    ...
    def observation(self, obs: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        for agent_id, agent_obs in obs.items():
            ...
            # 新向量观测
            pos_and_heading = [ego_pos[0], ego_pos[1], ego_heading, ego_speed]
            ...
            goal_pos_and_heading = [agent_obs["mission"]["goal_pos"][0], agent_obs["mission"]["goal_pos"][1], goal_heading[0] ,goal_distance[0]]
            ...
            if self.scenario == "1_to_2lane_left_turn_c":
                scenario_obs = np.array([1,0,0,0,0,0])
            elif self.scenario == "1_to_2lane_left_turn_t":
                scenario_obs = np.array([0,1,0,0,0,0])
            elif self.scenario == "3lane_merge_single_agent":
                scenario_obs = np.array([0,0,1,0,0,0])
            elif self.scenario == "3lane_cruise_single_agent":
                scenario_obs = np.array([0,0,0,1,0,0])
            elif self.scenario == "3lane_overtake":
                scenario_obs = np.array([0,0,0,0,1,0])
            elif self.scenario == "3lane_cut_in":
                scenario_obs = np.array([0,0,0,0,0,1])
            ...
            # waypoint位置方向与headingerror 10*4
            ...
            brief_final_obs = np.concatenate((brief_ego_pos_and_heading, brief_neighbor_pos_and_heading_1d,\
                                              brief_waypoint_pos_and_heading, brief_goal_pos_and_heading, norm_dist_from_center, scenario_obs), axis=0)
            # transformer观测 11*5
            brief_transformer_pos_and_heading_2d = np.array(transformer_pos_and_heading_2d, dtype=np.float32)
            # 共同观测 45
            brief_together_obs = np.concatenate((brief_waypoint_pos_and_heading, brief_goal_pos_and_heading, norm_dist_from_center, scenario_obs), axis=0)
            # 返回观测
            wrapped_obs.update(
                {
                    agent_id: {
