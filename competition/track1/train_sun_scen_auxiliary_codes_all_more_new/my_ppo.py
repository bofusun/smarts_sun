...
class PPO():
    def __init__(self, params):
        ...
        if self.norm_state:
            if params.actor_net_type == "mlp":
                ...
                self.actor_state_norm = AutoNormalization(self.actor_state_norm, [4, 112, 112], clip=10.0)
                self.critic_state_norm = AutoNormalization(self.critic_state_norm, [4, 112, 112], clip=10.0)      
        # if self.norm_reward:
    ...
    def train_step(self):
        ...
        for _ in range(self.buffer_capacity):
            ...
            for i in range(self.env_num):
                actor_state[i] = self.actor_state_norm(actor_state[i])
                critic_state[i] = self.critic_state_norm(critic_state[i])
            ...
            # 动作放入环境
            next_state, reward, done, infos  = self.train_envs.step(action.cpu().numpy())
        ...
        for i in reversed(range(self.buffer_capacity)):
            ...
            # return_value = advantage + self.replay_buffer[i].critic_value
    ...
    def eval_episode(self):
        ...
        while True:
            ...
            state, reward, done, info = self.test_env.step(action.cpu().numpy())
            all_reward += reward
            if done:
                break
        return all_reward
