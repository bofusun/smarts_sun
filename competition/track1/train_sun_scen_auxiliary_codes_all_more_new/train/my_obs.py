...
class FilterObs(gym.ObservationWrapper):
    ...
    def observation(self, obs: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        for agent_id, agent_obs in obs.items():
            ...
            if "1_to_3lane_left_turn_c" in self.scenario:
            ...
            #     scenario_obs = np.array([1])
                ...
                scenario_obs = np.array([1])
            elif "lanes_merge_single_agent" in self.scenario:
                scenario_obs = np.array([2])
            elif "lanes_cruise_single_agent" in self.scenario:
                scenario_obs = np.array([3])
            elif "lanes_overtake" in self.scenario:
                scenario_obs = np.array([4])
            elif "lanes_cut_in" in self.scenario:
