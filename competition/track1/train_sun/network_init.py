...
##############################################################
# MLP网络模型 continuous 三个连续动作
##############################################################
class Mlp_Actor_model(nn.Module):
    def __init__(self, feature_dim, action_dim, device):
        super(Mlp_Actor_model, self).__init__()
        ...
        self.sigma_output = nn.Linear(256, action_dim)
    ...
    def forward(self, input, action=None, determine=None):
        first = self.linear1(input)
        residual = first
        ...
        # 使用tanh来限制动作空间
        mu = self.mu_tanh(mu)
        mu_low = torch.tensor([0, 0, -1]).to(self.device)
        mu_high =torch.tensor([1, 1, 1]).to(self.device)
        mu = (mu_high - mu_low) / 2 * (mu+1) + mu_low
        # 获取sigma
...
# CNN Actor网络
#######################################################################################
class CNN_Actor_model1(nn.Module):
    def __init__(self, action_dim, device):
        super(CNN_Actor_model1, self).__init__()
        self.action_dim = action_dim
        self.device = device
    ...
    def forward(self, input, action=None, determine=None):
        ...
        # 三层卷积与两层全连接
        first_conv = self.conv1(input)
        # print("first_conv", first_conv.shape)
        second_conv = self.conv2(first_conv)
        # print("second_conv", second_conv.shape)
        third_conv = self.conv3(second_conv)
        # print("third_conv", third_conv.shape)
...
class CNN_Critic_net1(nn.Module):
    def __init__(self, device):
        super(CNN_Critic_net1, self).__init__()
        self.device = device
        self.conv1 = nn.Sequential(nn.Conv2d(4,32,kernel_size=8,stride=4,padding=2),
                                   nn.ReLU())
        self.conv2 = nn.Sequential(nn.Conv2d(32,64,kernel_size=4,stride=2,padding=1),
...
##############################################################
# MLP网络模型 continuous 两个连续动作
##############################################################
class Mlp_Actor_model_action2(nn.Module):
    def __init__(self, feature_dim, action_dim, device):
        super(Mlp_Actor_model_action2, self).__init__()
...
#######################################################################################
# CNN Actor网络 continuous 三个连续动作变为两个
#######################################################################################
class CNN_Actor_model_action2(nn.Module):
    def __init__(self, action_dim, device):
        super(CNN_Actor_model_action2, self).__init__()
        self.action_dim = action_dim - 1
        self.device = device
        self.conv1 = nn.Sequential(nn.Conv2d(4,32,kernel_size=8,stride=4,padding=2),
                                   nn.ReLU())
...
class Mlp_Actor_model_action_pose(nn.Module):
    ...
    def forward(self, input, action=None, determine=None):
        ...
        mu = self.mu_tanh(mu)
        """
        mu_low = torch.tensor([-0.1, 0, -0.1]).to(self.device)
        mu_high =torch.tensor([0.1, 2, 0.1]).to(self.device)
        mu = (mu_high - mu_low) / 2 * (mu+1) + mu_low
        """
        # 获取sigma
        sigma = F.softplus(self.sigma_output(second)) + 0.0001
...
##############################################################
# MLP网络模型 targetpose  第二种处理方式
##############################################################
class Mlp_Actor_model_action_pose2(nn.Module):
    def __init__(self, feature_dim, action_dim, device):
        super(Mlp_Actor_model_action_pose2, self).__init__()
        self.action_dim = action_dim
        self.device = device
        self.linear1 = nn.Sequential(nn.Linear(feature_dim, 256),
                                     nn.ReLU())
    ...
    def forward(self, input, action=None, determine=None):
        ...
        # 使用tanh来限制动作空间
        mu = self.mu_tanh(mu)
        mu_low = torch.tensor([0, -1]).to(self.device)
        mu_high =torch.tensor([2, 1]).to(self.device)
        mu = (mu_high - mu_low) / 2 * (mu+1) + mu_low
        # 获取sigma
        sigma = F.softplus(self.sigma_output(second)) + 0.0001
        var = sigma * sigma
...
class Mix_Actor_model_action_pose(nn.Module):
    ...
    def forward(self, cnn_input, vector_input, action=None, determine=None):
        # 向量特征提取
        first = self.linear1(vector_input)
        residual = first
        second = self.linear2(first)
        ...
        third_conv = third_conv.view(third_conv.size(0), -1)
        fc = self.fc(third_conv)
        cnn_feature = self.layer_norm(self.linear(fc) + fc)
        # 合并特征
...
class Mix_Critic_net(nn.Module):
    ...
    def forward(self, cnn_input, vector_input):
        # 向量特征提取
        first = self.linear1(vector_input)
        residual = first
        second = self.linear2(first)
        vector_feature = self.layer_norm(second + residual)
        # 图像特征提取
        first_conv = self.conv1(cnn_input)
        second_conv = self.conv2(first_conv)
        third_conv = self.conv3(second_conv)
        third_conv = third_conv.view(third_conv.size(0), -1)
        fc = self.fc(third_conv)
...
class All_Mix_Actor_model_action_pose(nn.Module):
    def __init__(self, feature_dim, action_dim, heads_num, key_dim, value_dim, layer_num, mlp_hidden_dim, device):
        ...
        self.device = device
        # transformer 特征提取
        self.encoder = Encoder(input_dim, 256, heads_num, key_dim, value_dim, layer_num, mlp_hidden_dim, device)
    ...
    def forward(self, cnn_input, vector_input, trans_input, judge_input, action=None, determine=None):
        # transformer特征提取
        trans_feature, encoder_self_attentions = self.encoder(trans_input, judge_input)
...
class All_Mix_Critic_net(nn.Module):
    ...
    def forward(self, cnn_input, vector_input, trans_input, judge_input):
        # transformer特征提取
        trans_feature, encoder_self_attentions = self.encoder(trans_input, judge_input)
        trans_feature = self.trans_fc(trans_feature)
        # 将output reshape output [batch_size, seq_len]
...
"""
#######################################################################################
# CNN Actor网络 targetpose   第二种处理方式
#######################################################################################
class CNN_Actor_model_action_pose2(nn.Module):
    def __init__(self, action_dim, device):
        super(CNN_Actor_model_action_pose2, self).__init__()
        self.action_dim = action_dim
        self.device = device
        self.conv1 = nn.Sequential(nn.Conv2d(4,32,kernel_size=8,stride=4,padding=2),
                                   nn.ReLU())
        ...
        # 初始化
        for i in range(len(self.conv1)):
            if type(self.conv1[i]) == nn.Linear:
                init.orthogonal_(self.conv1[i].weight, np.sqrt(2))
                self.conv1[i].bias.data.zero_()
        for i in range(len(self.conv2)):
            if type(self.conv2[i]) == nn.Linear:
                init.orthogonal_(self.conv2[i].weight, np.sqrt(2))
                self.conv2[i].bias.data.zero_()
        for i in range(len(self.conv3)):
            if type(self.conv3[i]) == nn.Linear:
                init.orthogonal_(self.conv3[i].weight, np.sqrt(2))
                self.conv3[i].bias.data.zero_()
        for i in range(len(self.linear)):
            if type(self.linear[i]) == nn.Linear:
