...
class PPO():
    ...
    def reshape_obs_new(self, obs):
        ...
        for i in range(self.env_num):
            ...
            print("i:",i)
            print("brief_together_obs:",obs[i]["brief_together_obs"])
            if np.nan in obs[i]["brief_together_obs"]:
                print(" have nan !")
            print("brief_transformer_pos_and_heading_2d:",obs[i]["brief_transformer_pos_and_heading_2d"])
            if np.nan in obs[i]["brief_transformer_pos_and_heading_2d"]:
                print(" have nan !")
            print("brief_judge:",obs[i]["brief_judge"])
            print("brief_scenario_obs:",obs[i]["brief_scenario_obs"])
    ...
    def train_step(self):
        ...
        return np.mean(return_rewards), np.mean(action_losses), np.mean(value_losses), np.mean(actor_supervise_losses), \
               ...
               np.mean(entropies), np.mean(kls), np.mean(lrs)
    ...
    def eval_episode(self, test_env):
        ...
        while True:
            ...
            if done:
                break
        return all_reward, total, actor_correct_num, critic_correct_num

    def eval_episodes(self, test_env, scenario_name):
        return_rewards = []
        for i in range(self.eval_num):
            all_reward, total, actor_correct_num, critic_correct_num = self.eval_episode(test_env)
            return_rewards.append(all_reward)
        actor_accuracy = actor_correct_num / total
        critic_accuracy = critic_correct_num / total
        self.logger.info('steps={}\t scenario{:s}\t test_return_reward={:.5f}\t actor_accuracy={:.5f}\t critic_accuracy={:.5f}'.format(self.counter, scenario_name, np.mean(return_rewards), actor_accuracy, critic_accuracy))
    ...
    def run_steps(self):
        ...
        while self.counter < self.max_steps:
            ...
            critic_supervise_loss, critic_supervise_accuracy, entropy, kl, lr = self.train_step()
            # 记录
            ...
            if self.counter % self.log_interval:
                ...
                self.logger.info(
                    ...
                    'kl={:.5f} \t lr={:.8f}' \
                    .format(self.counter, return_reward, action_loss, value_loss, actor_supervise_loss, \
...
if __name__ == "__main__":
    ...
    ###################################
    # 场景列表
    scenarios = ["1_to_3lane_left_turn_c", "1_to_3lane_right_turn_c", "1_to_3lane_left_turn_t", "lanes_merge_single_agent", \
                 "lanes_cruise_single_agent", "lanes_overtake", "lanes_cut_in"]
    scenario_nums = [2, 1, 1, 3, 3, 3, 2]
    # 是否使用sumo的可视化
    ...
    for i in range(env_num):
        train_env = MY_ENV_new(env_params, i, scenario_nums[i])  
