...
class MY_ENV_new():
    def __init__(self, env_params, env_class, env_count):
        self.scenarios = env_params.scenarios
        self.sumo_gui = env_params.sumo_gui 
        self.img_meters = env_params.img_meters 
        self.img_pixels = env_params.img_pixels
        ...
        self.env_list = []
        self.save_obs = [0 for i in range(self.env_count)]
        print("reseting")
        for i in range(self.env_count):
            temp_env = gym.make("smarts.env:multi-scenario-v0",
                                scenario = self.scenarios[self.env_class] + "_" + str(i+1),
                                img_meters = self.img_meters,
                                img_pixels = self.img_pixels,
                                action_space="TargetPose",
                                sumo_headless = not self.sumo_gui,  # If False, enables sumo-gui display.
    ...
    def reset(self):
        obs = self.env_list[self.count].reset()
        return obs
        
    def step(self, action):
        obs, reward, done, info = self.env_list[self.count].step({0: np.array(action)})
        if done:
            print("-------------------------------done------------------------------------")
            self.save_obs[self.count] = obs
            self.count = (self.count+1) % self.env_count
            if self.save_obs[self.count] == 0:
                obs = self.env_list[self.count].reset()
            else:
                obs = self.save_obs[self.count]
        return obs, reward, done, info
