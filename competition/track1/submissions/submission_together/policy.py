...
class Policy(BasePolicy):
    ...
    # 加载参数
    def load_param(self):
        actor_path = Path(__file__).absolute().parents[0] / "actor_net_100.pth"
    ...
    def act(self, obs: Dict[str, Any]):
        ...
        for agent_id, agent_obs in obs.items():
            ...
            # 生成动作
            with torch.no_grad():
                mu, sigma, action, log_prob, entropy, scenario_output = self.select_action(predicted, all_vector_state, rgb_state, together_vector_state, trans_vector_state, judge_state, determine=True)
