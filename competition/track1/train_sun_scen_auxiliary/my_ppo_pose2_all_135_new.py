...
class PPO():
    ...
    def train_step(self):
        ...
        for _ in range(self.buffer_capacity):
            ...
            # 生成动作
            with torch.no_grad():
                mu, sigma, action, log_prob, entropy, scenario_output = self.select_action(all_vector_state, rgb_state, together_vector_state, trans_vector_state, judge_state)
                # print("mu", mu)
        ...
        for _ in range(self.epoch):
            for index in BatchSampler(SubsetRandomSampler(range(self.buffer_capacity * self.env_num)), self.batch_size, drop_last=False):
                # 经过网络得到当前网络输出
                mu, sigma, action, log_prob, entropy, scenario_output = self.actor_net(all_vector_states[index], rgb_states[index], together_vector_states[index], trans_vector_states[index], judge_states[index], actions[index])
                ...
                supervise_loss = 0.5 * self.criterion(scenario_output, scenario_labels[index])
                # 统计分类情况
                _, predicted = torch.max(scenario_output.data, 1)
                total = scenario_labels[index].size(0)
                correct = (predicted.cpu() == scenario_labels[index].cpu()).squeeze().sum().numpy()
                # print("predicted", predicted)
                # print("scenario_labels[index]", scenario_labels[index])
                supervise_accuracy = correct / total
                # 计算kl散度
                ...
                supervise_losses.append(supervise_loss.cpu().detach().numpy())
                supervise_accuracies.append(supervise_accuracy)
                ...
                if self.train_together:
                    ...
                    (action_loss + value_loss + supervise_loss).backward()
        ...
        self.replay_buffer = []
        return np.mean(return_rewards), np.mean(action_losses), np.mean(value_losses), np.mean(supervise_losses), \
               np.mean(supervise_accuracies), np.mean(entropies), np.mean(kls), np.mean(lrs)
    ...
    def eval_step(self, obs):
        ...
        # 生成动作
        with torch.no_grad():
            mu, sigma, action, log_prob, entropy, scenario_output = self.select_action(all_vector_state, rgb_state, together_vector_state, trans_vector_state, judge_state, determine=True)
        # print("action", action)
        ...
        correct = (predicted.cpu() == scenario_label.cpu()).squeeze().sum().numpy()
    ...
    def eval_episode(self, test_env):
        ...
        correct_num = 0
        while True:
            action, correct = self.eval_step(state)
            ...
            if done:
                break
        return all_reward, total, correct_num

    def eval_episodes(self, test_env, scenario_name):
        return_rewards = []
        for i in range(self.eval_num):
            all_reward, total, correct_num = self.eval_episode(test_env)
            return_rewards.append(all_reward)
        accuracy = correct_num / total
        self.logger.info('steps={}\t scenario{:s}\t test_return_reward={:.5f}\t accuracy={:.5f}'.format(self.counter, scenario_name, np.mean(return_rewards), accuracy))
    ...
    def run_steps(self):
        ...
        if self.load_counter:
            ...
            supervise_losses = data['supervise_losses']
            supervise_accuracies = data['supervise_accuracies']
        ...
        while self.counter < self.max_steps:
            # 训练并记录训练数据
            return_reward, action_loss, value_loss, supervise_loss, supervise_accuracy, entropy, kl, lr = self.train_step()
            # 记录
            train_return_rewards.append(return_reward)
            action_losses.append(action_loss)
            value_losses.append(value_loss)
            supervise_losses.append(supervise_loss)
            supervise_accuracies.append(supervise_accuracy)
            ...
            if self.counter % self.log_interval:
                ...
                self.logger.info(
                    ...
                    'value_loss={:.3f} \t supervise_loss={:.3f} \t supervise_accuracy={:.3f}\t entropy={:.3f} \t kl={:.5f} \t lr={:.8f}' \
                    .format(self.counter, return_reward, action_loss, value_loss, supervise_loss, supervise_accuracy, entropy, kl, lr))
                t0 = time.time()
            ...
            if self.counter % self.log_interval == 0:
                save_info = {"train_return_rewards": train_return_rewards, "test_return_rewards": final_return_rewards,\
                             "action_losses": action_losses, "value_losses": value_losses, "supervise_losses": supervise_losses, \
                             "supervise_accuracies":supervise_accuracies, "entropies": entropies, "resource_kl":kls, "resource_lr": lrs}
