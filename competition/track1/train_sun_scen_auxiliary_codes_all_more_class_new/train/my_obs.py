            ...
            {agent_id: gym.spaces.Dict(
                    {
                        ...
                        "pos_and_heading": gym.spaces.Box(low=-1e10, high=1e10, shape=(9,), dtype=np.float32, ),
                        # rgb图像
                        ...
                        "reached_goal": gym.spaces.Box(low=-1e10, high=+1e10, shape=(1,), dtype=np.float32, ),
                    }
                )
    ...
    def observation(self, obs: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        for agent_id, agent_obs in obs.items():
            ...
            neighbor_pos_and_heading_2d = [[agent_obs['neighbors']['pos'][i][0], agent_obs['neighbors']['pos'][i][1], \
                                            ...
                                            for i in range(10)]
            brief_judge = [1]
            neighbor_judge = [0 if neighbor_pos_and_heading_2d[i][:4]==[0,0,0,0] else 1 for i in range(10)]
            brief_judge.extend([0 if neighbor_pos_and_heading_2d[i][:4]==[0,0,0,0] else 1 for i in range(10)])
            # 邻居车辆的距离
            ...
            # 场景信息
            ["1_to_3lane_left_turn_c", "1_to_3lane_right_turn_c", "1_to_3lane_left_turn_t", "lanes_merge_single_agent", \
             "lanes_cruise_single_agent", "lanes_overtake", "lanes_cut_in"]
            """
            if "1_to_3lane_left_turn_c_1" in self.scenario:
                scenario_obs = np.array([0])
            elif "1_to_3lane_left_turn_c_2" in self.scenario:
                scenario_obs = np.array([1])
            elif "1_to_3lane_right_turn_c" in self.scenario:
                scenario_obs = np.array([2])
            elif "1_to_3lane_left_turn_t" in self.scenario:
                scenario_obs = np.array([3])
            elif "lanes_merge_single_agent_1" in self.scenario:
                scenario_obs = np.array([4])
            elif "lanes_merge_single_agent_2" in self.scenario:
                scenario_obs = np.array([5])
            elif "lanes_merge_single_agent_3" in self.scenario:
                scenario_obs = np.array([6])
            elif "lanes_cruise_single_agent_1" in self.scenario:
                scenario_obs = np.array([7])
            elif "lanes_cruise_single_agent_2" in self.scenario:
                scenario_obs = np.array([8])
            elif "lanes_cruise_single_agent_3" in self.scenario:
                scenario_obs = np.array([9])
            elif "lanes_overtake_1" in self.scenario:
                scenario_obs = np.array([10])
            elif "lanes_overtake_2" in self.scenario:
                scenario_obs = np.array([11])
            elif "lanes_overtake_3" in self.scenario:
                scenario_obs = np.array([12])
            elif "lanes_cut_in_1" in self.scenario:
                scenario_obs = np.array([13])
            elif "lanes_cut_in_2" in self.scenario:
                scenario_obs = np.array([14])
            """
